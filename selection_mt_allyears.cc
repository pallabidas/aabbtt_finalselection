...
int main(int argc, char** argv){
    ...
    for (int i = 0; i < 108; i++){
        //i=0 nominal, 1-8 TES, 9-22 tauIDeff, 23-26 efaketauIDeff, 27-36 mufaketauIDeff, 37-40 muTES, 41-44 eleTES, 45-50 muonES, 51-54 trgsf, 55-58 btagsf, 59-82 jet, 83-94 recoil, 95-96 UES, 97-102 emb tautracking, 103-104 DY zpt, 105-106 ttbar toppt, 107 nonDY
        //1 bjet
    ...
    for (int i = 0; i < 73; i++){
        //tau anti-isolated region; i=0 nominal, 1-6 muonES, 7-10 trgsf, 11-14 btagsf, 15-38 jet, 39-50 recoil, 51-52 UES, 53-54 DY zpt, 55-56 ttbar toppt, 57-58 crosstrg fakefactor, 59-72 fake rate
        //1 bjet
    ...
    for(int i = 0; i < n; i++){
        ...
        for (int j = 0; j < 6; j++){
            ...
            var[0][1][j+17] = p_n(NN1b_muES[j],2);
            var[0][2][j+17] = p_n(NN1b_muES[j],2);
            var[0][3][j+17] = myshiftedmu[j].Pt();
            var[0][5][j+17] = mytt_shiftedmu[j].M();
            var[0][6][j+17] = m_btt_shiftedmu[j];
            
            var[1][0][j+17] = NN2b_muES[j];
            var[1][1][j+17] = p_n(NN2b_muES[j],2);
            var[1][2][j+17] = p_n(NN2b_muES[j],2);
        ...
        for (int j = 0; j < 24; j++){
            ...
            var[0][1][j+23] = p_n(NN1b_jetvar[j],2);
            var[0][2][j+23] = p_n(NN1b_jetvar[j],2);
            var[0][5][j+23] = mytt_jet[j].M();
            var[0][6][j+23] = m_btt_jet[j];
            
            var[1][0][j+23] = NN2b_jetvar[j];
            var[1][1][j+23] = p_n(NN2b_jetvar[j],2);
            var[1][2][j+23] = p_n(NN2b_jetvar[j],2);
        ...
        for (int j = 0; j < 12; j++){
            var[0][0][j+47] = NN1b_recoilvar[j];
            var[0][1][j+47] = p_n(NN1b_recoilvar[j],2);
            var[0][2][j+47] = p_n(NN1b_recoilvar[j],2);
            var[0][5][j+47] = mytt_recoil[j].M();
            var[0][6][j+47] = m_btt_recoil[j];
            
            var[1][0][j+47] = NN2b_recoilvar[j];
            var[1][1][j+47] = p_n(NN2b_recoilvar[j],2);
            var[1][2][j+47] = p_n(NN2b_recoilvar[j],2);
        ...
        for (int j = 0; j < 2; j++){
            ...
            var[0][1][j+59] = p_n(NN1b_UESvar[j],2);
            var[0][2][j+59] = p_n(NN1b_UESvar[j],2);
            var[0][5][j+59] = mytt_UES[j].M();
            var[0][6][j+59] = m_btt_UES[j];
            
            var[1][0][j+59] = NN2b_UESvar[j];
            var[1][1][j+59] = p_n(NN2b_UESvar[j],2);
            var[1][2][j+59] = p_n(NN2b_UESvar[j],2);
                ...
                if (sample!="data_obs"){
                    if (nbtag20==1 && trigger_region_nominal){
                        ...
                        if (sample!="embedded"){
                            ...
                            //btageff
                            for (int k = 0; k < 4; k++) h_iso[0][m][k+55]->Fill(var[0][m][0],weight_corr*trgsf*sf_tauid[0]*btagsf_syst[k]);
                        }
                        //trigger sf shifting
                ...
                if (sample!="data_obs"){
                    if (nbtag20==2 && trigger_region_nominal){
                        ...
                        if (sample!="embedded"){
                            ...
                            //btageff
                            for (int k = 0; k < 4; k++) h_iso[1][m][k+55]->Fill(var[1][m][0],weight_corr*trgsf*sf_tauid[0]*btagsf_syst[k]);
                        }
                        //trigger sf shifting
        ...
        if (byVVVLooseDeepVSjet_2 && !byMediumDeepVSjet_2 && isnonSignal && rejectEMBduplicate){//fake taus, do not need tau energy scale shifting, or tauid sf
            ...
            for (int m = 0; m < 7; m++){
                ...
                if (sample!="data_obs"){
                    if (nbtag20==1 && trigger_region_nominal){
                        ...
                        //btag sf
                        for (int k = 0; k < 4; k++) h_anti[0][m][k+11]->Fill(var[0][m][0],weight_corr*weight_fake*trgsf*xtrgfakefactor*btagsf_syst[k]);
            ...
            for (int m = 0; m < 8; m++){
                ...
                if (sample!="data_obs"){
                    if (nbtag20==2 && trigger_region_nominal){
                        ...
                        //btag sf
                        for (int k = 0; k < 4; k++) h_anti[1][m][k+11]->Fill(var[1][m][0],weight_corr*weight_fake*trgsf*xtrgfakefactor*btagsf_syst[k]);
    ...
    for (int j = 0; j < 7; j++){//loop over histogram variables ##############1b##############
        ...
        if (sample!="data_obs"){
            ...
            //shifted in isolated
            for (int i = 0; i < 14; ++i){
                h_iso[0][j][i+9]->SetName(name.c_str()+shape_tauideff[i]);
                h_iso[0][j][i+9]->Write();
            }
            for (int i = 0; i < 8; ++i){
                h_iso[0][j][i+1]->SetName(name.c_str()+shape_tauES[i]);
                h_iso[0][j][i+1]->Write();
            }
            if (sample!="embedded"){
                for (int i = 0; i < 24; ++i){
                ...
                for (int i = 0; i < 4; ++i){
                    h_iso[0][j][i+55]->SetName(name.c_str()+shape_btagsf[i]);
                    h_iso[0][j][i+55]->Write();
                }
            }
            for (int i = 0; i < 4; ++i){
                h_iso[0][j][i+51]->SetName(name.c_str()+shape_trgeff[i]);
            ...
            if (isnonHiggsMC && name!="Z"){
                ...
                h_iso[0][j][107]->SetName(nonDYMC+shape_nonDY[0]);
                h_iso[0][j][107]->Write();
            }
            ...
            if (isnonSignal){
                ...
                for (int i = 0; i < 4; ++i){
                    h_anti[0][j][i+11]->SetName(MC+fake+shape_btagsf[i]);
                    h_anti[0][j][i+11]->Write();
                }
                for (int i = 0; i < 4; ++i){
    ...
    for (int j = 0; j < 8; j++){//loop over histogram variables ##############2b##############
        ...
        if (sample!="data_obs"){
            ...
            //shifted in isolated
            for (int i = 0; i < 14; ++i){
                h_iso[1][j][i+9]->SetName(name.c_str()+shape_tauideff[i]);
                h_iso[1][j][i+9]->Write();
            }
            for (int i = 0; i < 8; ++i){
                h_iso[1][j][i+1]->SetName(name.c_str()+shape_tauES[i]);
                h_iso[1][j][i+1]->Write();
            }
            if (sample!="embedded"){
                for (int i = 0; i < 24; ++i){
                ...
                for (int i = 0; i < 4; ++i){
                    h_iso[1][j][i+55]->SetName(name.c_str()+shape_btagsf[i]);
                    h_iso[1][j][i+55]->Write();
                }
            }
            for (int i = 0; i < 4; ++i){
                h_iso[1][j][i+51]->SetName(name.c_str()+shape_trgeff[i]);
            ...
            if (isnonHiggsMC && name!="Z"){
                TString nonDYMC = "nonDYMC";
                h_iso[1][j][107]->SetName(nonDYMC+shape_nonDY[0]);
                h_iso[1][j][107]->Write();
            }
            ...
            if (isnonSignal){
                ...
                for (int i = 0; i < 4; ++i){
                    h_anti[1][j][i+11]->SetName(MC+fake+shape_btagsf[i]);
                    h_anti[1][j][i+11]->Write();
                }
                for (int i = 0; i < 4; ++i){
