...
if args.channel=="et" or args.channel=="mt":
    ...
    for i in range(len(dir)):
        ...
        if args.channel=="et":
            duplicate_up=file.Get(dir[i]).Get("embedded_CMS_EMB_tauidWP_et_"+args.year+"Up")
            duplicate_up.SetName("embedded_CMS_tauidWP_et_"+args.year+"Up")
            duplicate_up.Write()

            duplicate_down=file.Get(dir[i]).Get("embedded_CMS_EMB_tauidWP_et_"+args.year+"Down")
            duplicate_down.SetName("embedded_CMS_tauidWP_et_"+args.year+"Down")
            duplicate_down.Write()

        ########### prefiring weight for 2016 and 2017 only    
        if args.year=="2016" or args.year=="2017":
            mc_fake_syst_up=file.Get(dir[i]).Get("MC_fake_CMS_prefiring_"+args.year+"Up")
            fake_syst_up=file.Get(dir[i]).Get("data_obs_fake")
            fake_syst_up.Add(mc_fake_syst_up,-1)
            fake_syst_up.SetName("fake_CMS_prefiring_"+args.year+"Up")
            fake_syst_up.Write()
            
            mc_fake_syst_down=file.Get(dir[i]).Get("MC_fake_CMS_prefiring_"+args.year+"Down")
            fake_syst_down=file.Get(dir[i]).Get("data_obs_fake")
            fake_syst_down.Add(mc_fake_syst_down,-1)
            fake_syst_down.SetName("fake_CMS_prefiring_"+args.year+"Down")
...
if args.channel=="em":# do normalization on cloned histogram if it should be used multiple times (doing e.g. Integral() in loop will keep modifying along the way)
    ...
    for i in range(len(dir)):
        ...
        if is2b:
            qcd_ss_only=qcd_ss.Clone()
            qcd_ss_only.SetName("fake")
            qcd_ss_only.Write()
        else:
        ...
        ########### prefiring weight for 2016 and 2017 only
        if args.year=="2016" or args.year=="2017":
            mc_ss_syst_up=file.Get(dir[i]).Get("MC_ss_CMS_prefiring_"+args.year+"Up")
            mc_ssloose_syst_up=file.Get(dir[i]).Get("MC_ssloose_CMS_prefiring_"+args.year+"Up")
            ...
            if is2b:
                qcd_ss_only_syst_up=qcd_ss_syst_up.Clone()
                qcd_ss_only_syst_up.SetName("fake_CMS_prefiring_"+args.year+"Up")
                qcd_ss_only_syst_up.Write()
            else:
                scale=qcd_ss_syst_up.Integral()/qcd_ssloose_syst_up.Integral()
                qcd_syst_up=qcd_ssloose_syst_up
                qcd_syst_up.Scale(scale)
                qcd_syst_up.SetName("fake_CMS_prefiring_"+args.year+"Up")
                qcd_syst_up.Write()
            
            mc_ss_syst_down=file.Get(dir[i]).Get("MC_ss_CMS_prefiring_"+args.year+"Down")
            mc_ssloose_syst_down=file.Get(dir[i]).Get("MC_ssloose_CMS_prefiring_"+args.year+"Down")
            ...
            if is2b:
                qcd_ss_only_syst_down=qcd_ss_syst_down.Clone()
                qcd_ss_only_syst_down.SetName("fake_CMS_prefiring_"+args.year+"Down")
                qcd_ss_only_syst_down.Write()
            else:
                ...
                qcd_syst_down.SetName("fake_CMS_prefiring_"+args.year+"Down")
                qcd_syst_down.Write()
        ...
        for j in range(len(syst_name)):
            ...
            if is2b:
                qcd_ss_only_syst_up=qcd_ss_syst_up.Clone()
                qcd_ss_only_syst_up.SetName("fake_CMS_"+syst_name[j]+"_"+args.year+"Up")
                qcd_ss_only_syst_up.Write()
            else:
                scale=qcd_ss_syst_up.Integral()/qcd_ssloose_syst_up.Integral()
                qcd_syst_up=qcd_ssloose_syst_up
            ...
            if is2b:
                qcd_ss_only_syst_down=qcd_ss_syst_down.Clone()
                qcd_ss_only_syst_down.SetName("fake_CMS_"+syst_name[j]+"_"+args.year+"Down")
                qcd_ss_only_syst_down.Write()
            else:
                scale=qcd_ss_syst_down.Integral()/qcd_ssloose_syst_down.Integral()
                qcd_syst_down=qcd_ssloose_syst_down
        ...
        for j in range(len(syst_name)):
            ...
            if is2b:
                qcd_ss_only_syst_up=qcd_ss_syst_up.Clone()
                qcd_ss_only_syst_up.SetName("fake_CMS_"+syst_name[j]+"Up")
                qcd_ss_only_syst_up.Write()
            else:
                scale=qcd_ss_syst_up.Integral()/qcd_ssloose_syst_up.Integral()
                qcd_syst_up=qcd_ssloose_syst_up
            ...
            if is2b:
                qcd_ss_only_syst_down=qcd_ss_syst_down.Clone()
                qcd_ss_only_syst_down.SetName("fake_CMS_"+syst_name[j]+"Down")
                qcd_ss_only_syst_down.Write()
            else:
                scale=qcd_ss_syst_down.Integral()/qcd_ssloose_syst_down.Integral()
                qcd_syst_down=qcd_ssloose_syst_down
        ...
        ########### qcd estimation uncertainties: ssweights
        syst_name=["SScorrection","SSclosure","SSboth2D","osss"]
