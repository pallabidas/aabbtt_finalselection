    ...
    for(int i = 0; i < n; i++){
        ...
        if (((nbtag20==1 && fabs(beta_deepcsv_1)<2.4) or (nbtag20==2 && fabs(beta_deepcsv_1)<2.4 && fabs(beta_deepcsv_2)<2.4))){
            ...
            if (byMediumDeepVSjet_2){
                hist_m_et->Fill(m_et,weight_corr*weight_btag);
                hist_m_etb->Fill(m_etb,weight_corr*weight_btag);
                hist_e_pt->Fill(pt_1,weight_corr*weight_btag);
                hist_t_pt->Fill(pt_2,weight_corr*weight_btag);
                if (nbtag20==2) hist_m_etbb->Fill(m_etbb,weight_corr*weight_btag);
                
                //the 4 categories
                if (mt_taumet<60 && iso_1<0.1){
                    //category 1
                    if (m_etb<80 && mt_emet<40){
                        hist_m_et_1->Fill(m_et,weight_corr*weight_btag);
                    }
                    //category 2
                    if (m_etb>80 && m_etb<100 && mt_emet<50){
                        hist_m_et_2->Fill(m_et,weight_corr*weight_btag);
                    }
                    //category 3
                    if (m_etb>100 && m_etb<120 && mt_emet<50){
                        hist_m_et_3->Fill(m_et,weight_corr*weight_btag);
                    }
                    //category 4
                    if (m_etb>120 && mt_emet<40){
                        hist_m_et_4->Fill(m_et,weight_corr*weight_btag);
                    }
                }
            }
            ...
            if (byVVVLooseDeepVSjet_2 && !byMediumDeepVSjet_2){
                ...
                hist_m_et_qcd->Fill(m_et,weight_corr*weight_btag*weight_qcd);
                hist_m_etb_qcd->Fill(m_etb,weight_corr*weight_btag*weight_qcd);
                hist_e_pt_qcd->Fill(pt_1,weight_corr*weight_btag*weight_qcd);
                hist_t_pt_qcd->Fill(pt_2,weight_corr*weight_btag*weight_qcd);
                if (nbtag20==2) hist_m_etbb_qcd->Fill(m_etbb,weight_corr*weight_btag*weight_qcd);
                
                //the 4 categories
                if (mt_taumet<60 && iso_1<0.1){
                    //category 1
                    if (m_etb<80 && mt_emet<40){
                        hist_m_et_qcd_1->Fill(m_et,weight_corr*weight_btag*weight_qcd);
                    }
                    //category 2
                    if (m_etb>80 && m_etb<100 && mt_emet<50){
                        hist_m_et_qcd_2->Fill(m_et,weight_corr*weight_btag*weight_qcd);
                    }
                    //category 3
                    if (m_etb>100 && m_etb<120 && mt_emet<50){
                        hist_m_et_qcd_3->Fill(m_et,weight_corr*weight_btag*weight_qcd);
                    }
                    //category 4
                    if (m_etb>120 && mt_emet<40){
                        hist_m_et_qcd_4->Fill(m_et,weight_corr*weight_btag*weight_qcd);
                    }
                }
            }
        }
